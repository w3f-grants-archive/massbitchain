...
fn create_funded_user<T: Config>(name: &'static str, n: u32, balance_factor: u32) -> T::AccountId {
	let user = account(name, n, SEED);
...
fn register_candidates<T: Config>(count: u32) {
	...
	for who in candidates {
		...
		<ValidatorSet<T>>::register_as_candidate(RawOrigin::Signed(who).into()).unwrap();
	}
}
...
benchmarks! {
	...
	set_invulnerables {
		...
		assert_ok!(
			<ValidatorSet<T>>::set_invulnerables(origin, new_invulnerables.clone())
		);
	}
	verify {
	...
	set_desired_candidates {
		...
		assert_ok!(
			<ValidatorSet<T>>::set_desired_candidates(origin, max.clone())
		);
	}
	verify {
	...
	set_candidacy_bond {
		...
		assert_ok!(
			<ValidatorSet<T>>::set_candidacy_bond(origin, bond.clone())
		);
	}
	verify {
	...
	// worse case is when we have all the max-candidate slots filled except one, and we fill that one.
	...
	note_author {
		...
		<ValidatorSet<T> as EventHandler<_, _>>::note_author(author.clone())
	} verify {
		assert!(T::Currency::free_balance(&author) > 0u32.into());
	...
	new_session {
		...
		<ValidatorSet<T> as SessionManager<_>>::new_session(0)
	} verify {
